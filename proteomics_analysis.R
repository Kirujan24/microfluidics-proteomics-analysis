...
file_raw <- "Exosomes lcms raw.xlsx"

raw <- read_excel(file_raw)

raw2 <- raw %>%
  dplyr::mutate(Gene = stringr::str_extract(Description, "(?<=GN=)[^ ]+"))
...
intensity_cols <- grep(
  "Control|Early|Intermediate|Late",
  names(df),
  value = TRUE
)
...
plot_df <- expr_log %>%
  ...
  dplyr::mutate(
    ...
    Group = factor(Group, levels = c("EV markers", "Liver markers", "Housekeeping"))
  )
